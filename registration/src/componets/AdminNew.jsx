...
export default function AdminNew() {
  ...
  const fetchReferral = () => {
    fetch("http://localhost:8000/referral?status=New")
  ...
  const handleAgentSelection = (agentId) => {
    ...
    fetch(`http://localhost:8000/referral/${referralId}`, {
      ...
      .then((updatedReferral) => {
        // Update referral state to reflect the change
        const updatedReferrals = referral.map((u) =>
          u.id === updatedReferral.id
            ? { ...u, agentid: updatedReferral.agentid, status: updatedReferral.status }
            : u
        );
        setReferral(updatedReferrals);
        setShowModal(false);
      })
