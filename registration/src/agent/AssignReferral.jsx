...
const AssignReferral = () => {
  ...
  const fetchReferral = () => {
    ...
    fetch(`http://localhost:8000/referral?agentid=${agid}&&status=Inprocess`)
  ...
  const handleStatusUpdate = (id, newStatus) => {
    ...
    fetch(`http://localhost:8000/referral/${id}`, {
      ...
      .then((updatedReferral) => {
        // Remove the completed or rejected referral from the state
                            ...
                            >
                              {ref.status === "Inprocess" && (
                                <>
                                  ...
                                  <Dropdown.Item
                                    onClick={() => handleStatusUpdate(ref.id, "Rejected")}
                                  >
                                    Reject
                                  </Dropdown.Item>
                                </>
                              ...
                              {ref.status === "Complete" && (
                                <Dropdown.Item onClick={() => handleStatusUpdate(ref.id, "Rejected")}>
                                  Reject
                                </Dropdown.Item>
                              )}
                            </DropdownButton>
